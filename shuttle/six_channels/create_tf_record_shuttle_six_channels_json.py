...
flags = tf.app.flags
flags.DEFINE_string('labeled_path', '/media/seten/Datos/diego/TFM/dataset_tfm/shuttlecock/secuencias/train', 'Path where labeled images are')
flags.DEFINE_string('output_path', '/home/seten/TFM/data/train_shuttlecock_six_channels.record', 'Path to output TFRecord')
...
def create_tf_example(img_tuple, json_path, label_map_dict, ignore_difficult_instances=False):
    ...
    b,g,r = cv2.split(image_s)

    image_name  = Path(img_tuple[0]).name
    final_out_path_n = Path("/media/seten/Datos/diego/TFM/memoria/ejemplo_substraccion_canales/6_canales") / image_name
    final_out_path_s = Path("/media/seten/Datos/diego/TFM/memoria/ejemplo_substraccion_canales/6_canales_s") / image_name
    Image.fromarray(current_frame).save(final_out_path_n)
    Image.fromarray(image_s).save(final_out_path_s)
    ...
    for i,channel_im in enumerate(channels_s):
        tmp_im_name = "tmp_channel_json_{}.jpeg".format(i)
        Image.fromarray(channel_im).save(tmp_im_name)
        with tf.gfile.GFile(tmp_im_name, 'rb') as fid:
            encoded_channel = fid.read()
        encoded_channels.append(encoded_channel)
        encoded_channel_io = io.BytesIO(encoded_channel)
        image_channel = PIL.Image.open(encoded_channel_io)
        if image_channel.format != 'JPEG':
